#Spatstat demo from the spatstat Github repository.
#https://github.com/spatstat/spatstat/blob/master/demo/spatstat.R  (Press Ctrl and hover to follow the link!)
library(spatstat)
...
plot(X, main = "Point pattern on linear network (lpp)", show.window = FALSE)
...
plot(letterR, hatch = TRUE, hatchargs = list(texture = 8, spacing = 0.12), main = "Polygonal window with texture fill")

plot(amacrine, chars = c(1, 16), main = "plot(X, chars = c(1,16))")
plot(amacrine, cols = c("red", "blue"), chars = 16, main = "plot(X, cols=c(\"red\", \"blue\"))")
...
plot(longleaf, pch = 21, cex = 1, bg = colourmap(terrain.colors(128), range = c(0, 
    80)), main = "colourmap for numeric mark values")
...
plot(Z, main = "pixel image: logarithmic colour map", log = TRUE, col = rainbow(128, 
    end = 5/6))
contour(Z, main = "pixel image: contour plot", axes = FALSE)
plot(Z, main = "pixel image: image + contour plot")
contour(Z, add = TRUE)
persp(Z, colmap = terrain.colors(128), shade = 0.3, phi = 30, theta = 100, main = "pixel image: perspective plot")
...
X <- scaletointerval(as.im(function(x, y) {
    x
}, W = W))
plot(listof(L = L, D = D, X = X), main = "Multiple images")
pairs(L, D, X, main = "Multiple images: pairs plot")
persp(L, colin = D, theta = -24, phi = 35, box = FALSE, apron = TRUE, main = "Two images:\nperspective + colours", 
...
plot(as.im(function(x, y) {
    x^2 + y
}, W = square(1)), main = "as.im(function(x,y){x^2+y})")
...
plot(split(lansing), main = "split operation: split(X)", mar.panel = c(0, 0, 2, 0), 
    hsep = 1, pch = 3)

plot(longleaf, main = "Longleaf Pines data")
plot(cut(longleaf, breaks = 3), main = c("cut operation", "cut(longleaf, breaks=3)"))
...
plot(split(X, Z), main = "points split by tessellation", mar.panel = c(0, 0, 2, 2), 
    hsep = 1)

W <- square(1)
X <- as.im(function(x, y) {
    sqrt(x^2 + y^2)
}, W)
...
rh <- rhohat(X, D)
plot(rh, main = "Smoothed rate estimate", xlab = "Distance to nearest fault (km)", 
    legend = FALSE)
...
plot(te, main = c("Scan Statistic for redwood data", paste("p-value =", signif(te$p.value, 
    3))))
...
pa <- function(i, ...) {
    if (i == 1) 
        list(chars = c(".", "+"), cols = 1:2) else list(size = 0.5, pch = 16, col = coco)
}
plot(nnclean(X, k = 17), panel.args = pa, mar.panel = c(0, 1, 1, 0), nrows = 2, main = "Byers-Raftery nearest neighbour cleaning", 
    cex.title = 1.2)
...
X <- superimpose(runifpoint(300, letterR), runifpoint(50, W), W = W)
...
plot(listof(`Longleaf Pines data` = longleaf, `Nearest mark` = nnmark(longleaf), 
    `Kernel smoothing of marks` = Smooth(longleaf, 10), `Inverse distance weighted\nsmoothing of marks` = idw(longleaf)), 
    equal.scales = TRUE, halign = TRUE, valign = TRUE, main = "", mar.panel = 0.2 + 
        c(0, 0, 2, 2))
par(parsave)
...
b <- swedishpines[m]
arrows(swedishpines$x, swedishpines$y, b$x, b$y, angle = 12, length = 0.1, col = "red")

plot(swedishpines %mark% nndist(swedishpines), markscale = 1, main = "Stienen diagram", 
    legend = FALSE, fg = "blue")

plot(Gest(swedishpines), main = c("Nearest neighbour distance function G", "Gest(swedishpines)"), 
    legendmath = TRUE)
...
plot(Fest(swedishpines), main = c("Empty space function F", "Fest(swedishpines)"), 
    legendmath = TRUE)
...
L <- localL(ponderosa)
pL <- plot(L, lty = 1, col = 1, legend = FALSE, main = c("neighbourhood density functions", 
...
plot(Z12, col = topo.colors(128), main = c("smoothed", "neighbourhood density"), 
    cex.main = 0.8)
...
par(pty = "s")
mkc <- markcorr(amacrine, correction = "translate", method = "density", kernel = "epanechnikov")
plot(mkc, main = "Mark correlation function", legend = FALSE, cex.main = 0.8)
par(parsave)

par(mar = 0.2 + c(4, 4, 3, 1))
plot(alltypes(amacrine, markconnect), title = "Mark connection functions for amacrine cells")
...
plot(listof(`Emark(spruces)` = Emark(spruces), `Vmark(spruces)` = Vmark(spruces)), 
...
par(mfrow = c(1, 3))
X <- unmark(chicago)
plot(X, col = "green", cols = "red", pch = 16, main = "Chicago Street Crimes", cex.main = 0.75, 
    show.window = FALSE)
...
print(fit)
plot(fit, how = "image", se = FALSE, main = c("Inhomogeneous Poisson model", "fit by maximum likelihood", 
    "Fitted intensity"))
plot(fit, how = "image", trend = FALSE, main = c("Standard error", "of fitted intensity"))
...
plot(parres(fit, "D"), main = "Partial residuals from loglinear Poisson model", xlab = "Distance to nearest fault (km)", 
    ylab = "log intensity of gold", legend = FALSE)
legend("bottomleft", legend = c("partial residual", "loglinear fit"), col = c(1, 
    4), lty = c(1, 4))

par(mar = rep(0.2, 4), mfrow = c(1, 1))
fitT <- kppm(redwood ~ 1, clusters = "Thomas")
simT <- simulate(fitT)[[1]]
plot(listof(redwood, simT), main.panel = c("Redwood", "simulation from\nfitted Thomas model"), 
...
print(fit)
plot(fit, how = "image", main = c("Strauss model", "fit by maximum pseudolikelihood", 
    "Conditional intensity plot"))
# fitted interaction
plot(swedishpines)
fit <- ppm(swedishpines ~ 1, PairPiece(c(3, 5, 7, 9, 11, 13)))
plot(fitin(fit), legend = FALSE, main = c("Pairwise interaction model", "fit by maximum pseudolikelihood"))

# simulation
par(mfrow = c(1, 1), mar = 0.5 + c(0, 0, 2, 0))
Xsim <- rmh(model = fit, start = list(n.start = 80), control = list(nrep = 100))
plot(listof(swedishpines, Xsim), main = "", main.panel = c("Swedish Pines", "Simulation from\nfitted Strauss model"), 
...
plot(swedishpines)
fit <- ppm(swedishpines ~ 1, Strauss(r = 7))
plot(Kcom(fit), cbind(iso, icom, pois) ~ r, legend = FALSE, main = "model compensators")
legend("topleft", legend = c("empirical K function", "Strauss model compensator of K", 
...
plot(rpoispp(100))
plot(rpoispp(function(x, y) {
    1000 * exp(-3 * x)
}, 1000), main = "rpoispp(function)")
...
Xg <- rmh(list(cif = "geyer", par = list(beta = 1.25, gamma = 1.6, r = 0.2, sat = 4.5), 
    w = c(0, 10, 0, 10)), control = list(nrep = 10000), start = list(n.start = 200))
...
showoffK <- function(Y, current, ..., fullpicture, rad) {
    plot(fullpicture, main = c("Animation using `applynbd'", "explaining the K function"))
    points(Y, cex = 2)
    u <- current
